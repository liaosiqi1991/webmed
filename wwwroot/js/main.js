    ...
    try {
        $.ajax({
            ...
            success: function (redata) {
                ...
                $("#p2").html("当前共有病案记录数量：" +n4 +"。其中未接收："+ n3 +"条、已接收" + n2 + "条、已编目："+ n1 +"条。");
...
function modify() {
    ...
    mmecinfo= row[0].病人ID + "_" + row[0].主页ID;
...
//显示或者隐藏查询窗口 type 0 隐藏  1 显示
function ShowQueryWindow() {

    var buttonText = $('#btnWindow').html()
    ...
    if (buttonText == '&lt;&lt;') {
        ...
        //>>原状态已经隐藏 调整为显示
        $('#btnWindow').html('&lt;&lt;')
        $("#left").attr("style", "display:inline;");
        $("#right").attr("style", "width:940px;");
    }
}
